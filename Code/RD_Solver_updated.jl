...
@time begin
    ...
    time_series = plot(time_steps, average_c, xlabel="Time", ylabel="Mean", lw=8, xlabelfontsize=20, ylabelfontsize=20,
        titlefontsize=12, legendfontsize=12, tickfontsize=16, yticks=0:0.25:1.1, xticks=0:20:tfinal, ylim=(-0.01, 1.01), xlim=(-1, tfinal*1.02), legend=false) #, label="Mean Consensus Makers"
    plot!(time_steps, average_g, lw=8)
    plot!(time_steps, average_z1, lw=8)
    ...
    display(time_series)
    savefig("TS,M=$M,kappa=$κ,lambda=$λ,s=$s.pdf")
end