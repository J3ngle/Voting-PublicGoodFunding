   ...
   # M = 0.05#/L^2 # M/L^2 in the main text , L^2 moved to Directed movement to account for changing M
    ...
    function pg_system!(du, u, p, t)
        ...
        for (pop, U, du_pop) in (
            ...
            eU  = exp.(κ .* U)
            emU = inv.(eU)                       
            Sw  = dot(vec(pop), vec(emU))  # Sw = Σ_(j,k) pop(j,k)*eU(j,k)    
            Se  = sum(eU)      # Se = Σ_(j,k) emU(j,k)                   
            du_pop .+= M ./ (L^2) .* (eU .* Sw .- pop .* emU .* Se) 
        end
    ...
    maxvar        = zeros(iter)
    averagevar_c  = zeros(iter)   # spatial variance of consensus-maker density
    averagevar_g  = zeros(iter)   # spatial variance of gridlocker density
    averagevar_z1 = zeros(iter)   # spatial variance of Party-1 zealot density
    averagevar_z2 = zeros(iter)   # spatial variance of Party-2 zealot density

    for i in 1:iter
        M_val = M_i_array[i]
        averagevar_P    = zeros(P)
        maxvar_P        = zeros(P)
        averagevar_c_P  = zeros(P)
        averagevar_g_P  = zeros(P)
        averagevar_z1_P = zeros(P)
        averagevar_z2_P = zeros(P)
        ...
        for p in 1:P
            ...
            var_c_timeseries  = zeros(length(sol))
            var_g_timeseries  = zeros(length(sol))
            var_z1_timeseries = zeros(length(sol))
            var_z2_timeseries = zeros(length(sol))
            for t in 1:length(sol)
                ...
                v  = c .* v_c .+ g .* v_g .+ z1
                var_timeseries[t]    = var(vec(v))   # spatial variance of vote across grid
                var_c_timeseries[t]  = var(vec(c))   # spatial variance of consensus-maker density
                var_g_timeseries[t]  = var(vec(g))   # spatial variance of gridlocker density
                var_z1_timeseries[t] = var(vec(z1))  # spatial variance of Party-1 zealot density
                var_z2_timeseries[t] = var(vec(z2))  # spatial variance of Party-2 zealot density
            end

            averagevar_P[p]    = mean(var_timeseries)
            maxvar_P[p]        = maximum(var_timeseries)
            averagevar_c_P[p]  = mean(var_c_timeseries)
            averagevar_g_P[p]  = mean(var_g_timeseries)
            averagevar_z1_P[p] = mean(var_z1_timeseries)
            averagevar_z2_P[p] = mean(var_z2_timeseries)
        end

        # FIX 3: Average over all repetitions *after* the p-loop
        averagevar[i]    = mean(averagevar_P)
        maxvar[i]        = mean(maxvar_P)
        averagevar_c[i]  = mean(averagevar_c_P)
        averagevar_g[i]  = mean(averagevar_g_P)
        averagevar_z1[i] = mean(averagevar_z1_P)
        averagevar_z2[i] = mean(averagevar_z2_P)
    end
    
    # Plot vote variance results
    ...
    # Plot strategy variance results
    strategy_var_plot = plot(M_i_array, averagevar_c,
        label="Consensus (c)", lw=4, linestyle=:solid,
        xlabel="M", ylabel="Mean Variance of Strategy Density",
        xlabelfontsize=20, ylabelfontsize=20, titlefontsize=12,
        legendfontsize=12, tickfontsize=16)
    plot!(M_i_array, averagevar_g,  label="Gridlocker (g)",      lw=4, linestyle=:dash)
    plot!(M_i_array, averagevar_z1, label="Zealot Party 1 (z1)", lw=4, linestyle=:dot)
    plot!(M_i_array, averagevar_z2, label="Zealot Party 2 (z2)", lw=4, linestyle=:dashdot)
    display(strategy_var_plot)
    #savefig("StrategyVariance_DifferentM.pdf")
