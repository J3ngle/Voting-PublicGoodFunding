...
@time begin
    ...
    invdist = inv.(Dist)                                      
    invdist[diagind(invdist)] .= 0       
    Dist[diagind(Dist)] .= 0  #Distance from a node to itself is set to zero, Good 
    A =sum(Dist)  #Normalization factor for Distance Dependent movement, Good                     
    #Initial conditions
