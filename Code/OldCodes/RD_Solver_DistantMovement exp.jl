...
# Discrete diffusion (periodic boundaries) for N,S,E,W neighbors
...
savefig("Figure5ConsensusMakers,Nx=$Nx,Dc=$D_c,M_c=$m_c,lambda=$λ,s=$s,T=$tfinal.pdf")
display(plot(p2, axis=false, framestyle=:none, ticks=false,size=(625, 625))) #Gridlockers
savefig("Figure5Gridlockers,Nx=$Nx,Dg=$D_g,M_g=$m_g,lambda=$λ,s=$s,T=$tfinal.pdf")
display(plot(p3, axis=false, framestyle=:none, ticks=false,size=(625, 625))) #Zealots of party 1
savefig("Figure5Zealots1,Nx=$Nx,Dz1=$D_z,M_z1=$m_z,lambda=$λ,s=$s,T=$tfinal.pdf")
display(plot(p4, axis=false, framestyle=:none, ticks=false,size=(625, 625))) #Zealots of party 2
savefig("Figure5Zealots2,Nx=$Nx,Dz2=$D_z2,M_z2=$m_z2,lambda=$λ,s=$s,T=$tfinal.pdf")
display(plot(p5, axis=false, framestyle=:none, ticks=false,size=(625, 625))) #Population
savefig("Figure5Population,Nx=$Nx,Dc=$D_c,M_c=$m_c,lambda=$λ,s=$s,T=$tfinal.pdf")
display(plot(p6, axis=false, framestyle=:none, ticks=false, size=(625,625))) #Vote
savefig("Figure5Vote,Nx=$Nx,Dc=$D_c,M_c=$m_c,lambda=$λ,s=$s,T=$tfinal.pdf")
...
time_series = plot(time_steps, average_c, xlabel="Time", ylabel="Mean",lw=8, xlabelfontsize=20, ylabelfontsize=20,
     titlefontsize=12, legendfontsize=12, tickfontsize=16, yticks=0:0.25:1.1, xticks=0:tfinal/5:tfinal, ylim=(0,1), xlim=(0,tfinal), legend=false) #, label="Mean Consensus Makers"
